...
public interface LeaveApplyDao extends CrudRepository<LeaveApply, Integer> {
    ...
    @Query(value = "SELECT l.`id`, l.`days`, l.`empid`,e.name,e.designation,e.emp_code,l.remarks, l.`from`, l.`reason`,l.`status`, l.`to`, l.`type_ofleave` FROM `leaves` l JOIN employee e ON e.id=l.empid ",nativeQuery = true)
    ...
    List<LeaveApply> searchLeaves(@Param("empid") Integer empid);
